...
public class EqualSum {
    ...
    public static boolean equalSum(List<Integer> list) {
        ...
        return equalSumHelper(list, -1, 0, 0);
    }

    private static boolean equalSumHelper(List<Integer> list, int start, int sum1, int sum2) { // add any parameters
        // base case
        if(sum1 == sum2 && start >= list.size()-1)return true;
        else if(start >= list.size()-1)return false;
        // recursive step
        else return equalSumHelper(list, start+1, sum1 + list.get(start+1), sum2)
        || equalSumHelper(list, start+1, sum1, sum2 + list.get(start+1));
    }
}
